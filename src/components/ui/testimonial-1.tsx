...
import { ArrowUp, ExternalLink, GraduationCap, Brain, Trophy, Award } from "lucide-react";
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="sm:flex grid grid-cols-2 gap-8 bg-white/40 dark:bg-zinc-900/40 backdrop-blur-xl mt-12 w-full mx-auto px-8 py-10 border rounded-2xl border-white/20 dark:border-white/5 shadow-2xl relative overflow-hidden">
          ...
          {stats.map((stat, index) => {
            const isExternal = stat.external;
            const LinkComponent = isExternal ? 'a' : Link;
            const linkProps = isExternal ? { href: stat.href, target: "_blank", rel: "noopener noreferrer" } : { href: stat.href };
            return (
              <LinkComponent
                key={stat.label}
              ...
              >
                <div className="flex flex-col items-center justify-center relative h-full group">
                  ...
                  <div className="flex flex-col items-center text-center transition-all duration-300 group-hover:opacity-40">
                    ...
                    <div className="text-3xl sm:text-4xl font-black text-neutral-900 dark:text-white tracking-tighter flex items-baseline mb-1">
                      <Counter value={stat.value} decimals={stat.decimals} />
                      <span className="text-lg sm:text-2xl ml-0.5">{stat.suffix}</span>
                    </div>
                    <span className="text-[10px] sm:text-xs font-semibold text-neutral-500 dark:text-zinc-400 uppercase tracking-wider">
                      {stat.label}
                    </span>
                  </div>
