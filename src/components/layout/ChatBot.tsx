...
function ChatWindow({ onClose }: { onClose: () => void }) {
    ...
    useEffect(() => {
        const handleClickOutside = (event: MouseEvent) => {
            ...
            // Ignore click if it's inside the chat window or on a toggle button
            const isToggleButton = target.closest(".chatbot-toggle-btn");
            if (
        ...
        // Delay attaching to avoid closing immediately if the click that opened the chat is still bubbling
        ...
        <>
            ...
            {!headless && (
                <motion.button
                    ...
                    whileTap={{ scale: 0.95 }}
                    className={cn(
                        "chatbot-toggle-btn relative p-3 rounded-full transition-all group",
